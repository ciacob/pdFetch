 ...
 *
 * @returns {Number} Returns `0` if program completed normally, `1` for expected early exits
 *          (e.g., the `--help` argument was given), or `2` if program exited because of an
 *          error.
 */
async function mainExec(input, monitoringFn = null) {
  ...
  // Extract input data
  const { configDefaults, configFileTemplate, appArgDefaults, appArgs } = input;
  ...
  if (args.init_config) {
    ...
    return 1; // Early exit for config initialization
  }

  // Load default profile
  const configFilePath = path.join(getUserHomeDirectory(), "pdFetch.config");
  const defaultProfileData = getConfigData(configFilePath, "default", $m) || {};
  ...
  const finalInputData = mergeData(defaultProfileData, profileData, args);
  ...
  try {
    ...
    // Handle storage mode
    const listFilePath = path.join(finalInputData.output_dir, "file_list.json");
